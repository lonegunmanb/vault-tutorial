...
拔插件"那个通用心智模型是个例外——Vault 内置了**两个**这样被特殊对
待的引擎：本章主角 `cubbyhole/`，以及承担集群身份中枢职责的
`identity/`（详见 [3.6 Identity 引擎](/ch3-identity)）。它们的共同点
是承载的不是业务数据，而是 Vault 自身的运行时基础设施——Cubbyhole 撑
着 Token 体系，Identity 撑着身份与 OIDC 体系——所以挂载点级别的操作
全部被锁。
...
- **[2.7 Response Wrapping](/ch2-response-wrapping)**：本章 §7 把那
  一节的"黑箱"解开
- **[3.1 机密引擎概览](/ch3-secrets-engines)** & **[3.6 Identity 引擎](/ch3-identity)**：
  Cubbyhole 是该章"通用挂载模型"的两个例外之一，另一个是
  `identity/`——记住这两个例外比记住通用规则更重要
