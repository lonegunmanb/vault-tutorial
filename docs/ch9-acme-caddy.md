...
## 5. 证书轮转：到期自动续期与管理员手动吊销

第 2.3 节已经点明 ACME 的真正红利不在于『一次签发』而在于『静默续期』。本节把这条结论拆成两个**生产里一定会遇到**的具体场景，并提前讲清楚 Caddy 与 Vault 在每种场景里各自承担什么职责，以便后面的实验不再纠缠机制只看现象。

### 5.1 场景 A：到期前的『静默续期』

ACME 客户端拿到一张证书后，会持续盯着它的 `notAfter` 字段：

1. 当寿命走过约 **2/3** 时，客户端就会**主动**重跑一遍 §2.2 的三步对话，向 ACME 服务器再要一张新证书；
2. 拿到新证书后，客户端原子地把磁盘上的 `*.crt` / `*.key` 替换掉，并**热加载**新证书装入正在监听 :443 的 TLS 栈，期间不掉线；
3. 整个过程**不需要任何人介入**，也**不需要 Vault 主动通知**——监控到期时间点的责任完全在客户端一侧。

这一机制的工程含义：内部证书的有效期可以**断崖式缩短**（从一年缩到 24 小时甚至更短），泄露伤害窗口随之收窄；运维不再需要为续期值班。

> **续期失败会怎样？** 续期是『提前 1/3 寿命』就开始尝试的，给故障留出了相当大的重试窗口。即便 Vault 在某一刻不可用，客户端会在剩余寿命内反复重试；只要在**剩余 1/3 寿命**用尽前 Vault 恢复，续期就能成功。这也是为什么 Vault 集群本身的高可用建设（[6 章](/ch6-config-overview)）必须配套到位——一旦故障窗口超过证书剩余寿命，所有依赖它的下游服务会一并失效。

### 5.2 场景 B：管理员『手动吊销』

私钥泄露、人员离职、合规审计要求作废某张证书……这些都是 ACME 客户端不会主动感知到的事件，必须由**管理员在 Vault 一侧主动发起吊销**。Vault PKI 引擎为此提供了一组 API：

| 端点 | 作用 |
| --- | --- |
| `vault list pki_int/certs` | 列出所有签发过的证书序列号（含已吊销的） |
| `vault write pki_int/revoke serial_number=<sn>` | 吊销指定序列号的证书；Vault 会立即把它写进自己维护的 **CRL（Certificate Revocation List）** |
| `vault read pki_int/cert/crl` / `pki_int/issuer/<id>/crl/der` | 读出当前 CRL；任何 TLS 客户端都可以下载它来判断对端证书是否被吊销 |
| `pki_int/ocsp` | OCSP 响应端点，提供『某序列号是否被吊销』的单点查询 |

吊销之后，下游 Caddy 进程要走完一次完整的『让新证书顶上来』需要两件事衔接：

1. **Caddy 自己感知到旧证书已经无效**——Caddy 内置的 OCSP staple 维护循环会周期性地从 `pki_int/ocsp` 拉一次状态，发现吊销后立刻**触发一次 ACME 续期**；
2. **新证书合法地从 Vault 拿到手**——重跑一次 §2.2 的三步对话即可，因为 ACME 协议的『下单 / 挑战 / 结单』本身不区分『首签』与『吊销后重签』。

> **教学场景下如何把这条链路在几分钟内复现？** 生产里 OCSP 拉取与续期检查都按分钟级周期跑，等待相对较长。在交互式实验里我们用两个加速手段：（a）把 `pki_int/roles/<role>` 的 `max_ttl` 临时调到 2~5 分钟，让 ACME 客户端在一两分钟内就触发『过 2/3 寿命』续期；（b）直接清掉 Caddy 数据目录里的 `certificates/` 子目录然后重启容器，强制它重新走一遍下单流程。两种方式都不改变 ACME 协议本身的运作机理——它们只是把『等几个小时』压缩到『等几十秒』，便于学员**亲眼看到** 续期与重签后的『新序列号、新 `notBefore` / `notAfter`』。

### 5.3 学员需要在终端里亲眼看到的三件事

为防止『听完原理还是不踏实』，本节实验配套的 Step 4 会让学员**在同一个 Caddy 进程上**依次复现：

- 一张当前 in-use 的证书的**序列号**与有效期；
- 缩短 TTL 并触发一次续期之后，序列号变化、`notAfter` 显著推后；
- 在 Vault 里 `revoke` 该序列号、确认它出现在 CRL 里、再让 Caddy 重新申请——拿到的新证书**序列号又变了一次**、且旧序列号永久留在 CRL 中。

---

## 6. 实验整体路线图

本节配套了一个 Killercoda 实验，四步走：
...
4. **Step 4**：把上一节『证书轮转』的两种触发方式都跑一遍——先把角色 TTL 缩短并观察 Caddy **自动续期**（序列号、有效期都换了一组）；再在 Vault 里 `revoke` 一次旧序列号、确认它进入 CRL、并让 Caddy 重新申请到一张全新证书。整个过程依然**不需要任何人手动签发或分发证书**。
...
6. **轮转触发器有两种**——『到期前 2/3 寿命』由 ACME 客户端**主动**驱动；『管理员手动吊销』由 Vault 一侧的 `pki_int/revoke` 写入 CRL、Caddy 通过 OCSP staple 巡检感知后再触发同一套续期流程；两种触发器走的都是同一套 `newOrder → challenge → finalize` 协议管线，对运维而言**只是一句命令的差别**；
7. **边界**——ACME 不解决申请方的合法性问题（用 PKI role 的 `allowed_domains` 与 ACME EAB 收口），不解决客户端被攻陷问题，不替代 Vault 集群自身的高可用建设。

掌握这份清单后，可在动手实验中亲自把一台 Caddy 接到 Vault PKI 的 ACME 服务上，依次复现『纯 HTTP 反例 → PKI + ACME 一次性配齐 → 全自动 HTTPS → 到期续期 + 吊销重签』四个阶段。

---

## 9. 动手实验

本节配套了一个 Killercoda 实验：学员将在单台 Killercoda 主机上启动 dev 模式的 Vault，先把 Caddy 以纯 HTTP 跑一遍作为对照，再启用并配置 PKI + ACME，让 Caddy 全自动从 Vault 拿到证书并对外提供 HTTPS，最后**亲眼观察证书在到期前自动续期、以及在 Vault 一侧吊销后被自动重签**两种轮转触发器的实际效果。
