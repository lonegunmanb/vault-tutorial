...
> **绘图指令（待生成）**：
> 风格：手绘风格、线条简洁、可爱、易于理解，使用现实事物比喻。
> 内容：左侧画一个拿着钥匙的小机器人（ACME 客户端），中间画一座邮局（Vault ACME CA），右侧画一栋带门牌号的房子（目标域名）。流程分三步：① 小机器人向邮局申请"我要证明我住在这栋房子里"；② 邮局说"请在门口挂一块写着暗号的牌子"（挑战）；③ 邮局派邮递员去房子门口检查牌子（验证），确认后盖章发放证书。
> **三种挑战方式三选一**：在画面下方画一个"三选一"分镜栏，三个并排小格子分别展示同一栋房子在不同挑战方式下的画法（每张子图只挂一块对应颜色的暗号牌、其余位置留空），让读者一眼看出"客户端按场景选一种"，避免误解为同时挂三块：
> - 子图 A — HTTP-01：蓝色暗号牌挂在大门上（标 `/.well-known/acme-challenge/`）
> - 子图 B — DNS-01：绿色暗号牌插在信箱（DNS 服务器）上（标 TXT 记录）
> - 子图 C — TLS-ALPN-01：橙色暗号牌贴在 443 窗户上（标 ALPN 扩展）
> 每块牌子上画一个⭐并写"暗号"两字。三块子图共用一个图例气泡："⭐ = challenge token（暗号）；客户端三选一即可"。
