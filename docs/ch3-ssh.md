...
CA 模式的前提是"目标主机的 sshd 我能改配置"。如果目标主机出于某种
原因**只支持密码认证**（老旧设备、合规要求、跨厂商混合环境），CA 模
式就用不上。这时候就轮到 OTP 模式。
...
| 现象 | 根因 | 解决 |
| :--- | :--- | :--- |
| `Permission denied (publickey)`，sshd 日志里 `name is not a listed principal` | role 没设 `default_user`，签出的证书里没有合法 principal | 在 role 里设 `default_user=ubuntu`，或签发时显式 `valid_principals=...` |
