...
`raft/peers.json` 是 Raft 这一层的"成员关系兜底文件"——它直接告诉 Vault："集群现在就只剩这几个节点，别再等其它节点回来了，立刻按这份名单重新选主。"

正因如此，它有一个非常危险的副作用：**那些原本还没在多数派之间达成一致、按 Raft 协议本应被丢弃的写入，会被强行当成"已生效"接受下来**。换句话说，使用 `peers.json` 等同于人工宣布"我决定相信这台幸存节点上的所有数据，哪怕其中一部分理论上不该提交"——这是一种以"可能少量数据失真"换取"集群重新可用"的取舍。

所以使用该文件的两条铁律是：

- **只在确实没有其它办法**（节点起不来、`remove-peer` 也调不通）时才用，作为最后兜底；
- **绝对不要把它做成自动化**——例如不要写一个"每天凌晨扫描一遍存活节点、自动生成新的 `peers.json` 覆盖到所有节点"的 cron 脚本。这种做法乍看像是"让集群自愈"，实际上每跑一次都会触发上面那种"强行接受未提交数据"的副作用，等同于让数据每天都有一次无声受损的机会。`peers.json` 必须由人工在故障现场放上去，集群恢复后立刻删除，永不让它在正常运行的集群中出现第二次。
