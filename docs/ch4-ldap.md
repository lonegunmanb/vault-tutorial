...
`binddn` 应当是只具备搜索用户和组所需权限的低权限账号，而不是目录管理员账号；虽然官方文档只要求它能执行用户和组搜索，但从最小权限原则看，它不应拥有修改密码、创建用户或删除条目的能力。本章配套实验为了简化 OpenLDAP ACL 配置，`binddn` 直接使用了 `cn=admin`；这只是教学便利，请勿照搬到生产环境。
