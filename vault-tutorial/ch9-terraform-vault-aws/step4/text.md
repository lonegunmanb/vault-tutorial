...
这里的失败链路非常重要：Operator 仍然能向 Vault 要到一对动态 IAM key，因为 role 还允许 `iam:*`；但这对新 key 已经没有 `ec2:*`，所以 AWS provider 调 EC2 API 时被 LocalStack 拒绝。真实 AWS 中也是同一类结果。
...
- [ ] Admin 工作区 `terraform destroy` 成功清理 Vault backend 与 LocalStack IAM user
