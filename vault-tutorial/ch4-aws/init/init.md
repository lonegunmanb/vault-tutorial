...
- **Step 1**：启动 MiniStack，`vault auth enable aws`，把
  `config/client` 的 `sts_endpoint` / `iam_endpoint` 指向 MiniStack；
  设 `iam_server_id_header_value`
- **Step 2**：建一个 `auth_type=iam` 的 role 并**真正完成一次 iam
  登录**——`vault login -method=aws` 走完"客户端签名 → Vault 转发
...
认证流程在本地能跑通。但它**不模拟 EC2 实例的 PKCS#7 instance
