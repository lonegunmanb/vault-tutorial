...
echo "   构建模拟遗留应用的 Go 二进制，并生成 Consul-Template 与 Vault Agent 配置文件)"
echo "================================================="
...
echo "✅ 环境已就绪！"
echo ""
echo "📦 已安装：vault $(vault version 2>/dev/null | head -1 | awk '{print $2}')，consul-template $(consul-template -v 2>&1 | head -1 | awk '{print $3}')，psql $(psql --version | awk '{print $3}')，go $(go version | awk '{print $3}')"
