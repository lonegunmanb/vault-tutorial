...
本实验配套 [6.9 节正文](/ch6-user-lockout)：学员此时已经在概念层理解了"user lockout 默认启用、threshold/duration/counter_reset 三个参数构成状态机、配置块按 `all` / `userpass` / `ldap` / `approle` 划分作用域、`VAULT_DISABLE_USER_LOCKOUT` 处于禁用优先级链最顶端、`/sys/locked-users` 提供查询与幂等解锁"这套机制，本实验把其中可观察的部分变成可在终端里直接复现的现象：
