...
- 终止当前 leader 进程触发 Raft 重新选举，验证新 leader 在剩余两节点间产生，且转发路径与重定向路径均自动跟随新 leader 迁移、数据完整保留。

至此，6.5 节正文中"活跃节点抢锁、待命节点转发或重定向、`api_addr` 与 `cluster_addr` 各司其职"这一组核心概念，已从文档描述转化为可在终端中复现的实际现象。
