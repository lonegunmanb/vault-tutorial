...
预期可观察到依次打印的两段 HTTP 报文：第一段为 standby 返回的 `307` 与 `Location` 头；第二段为 `curl` 携带同一 token 向 active 重新发起请求并收到的 `200 OK` 与实际数据。这正是"客户端重定向"路径的完整 4 步：客户端 → standby → 307 → 客户端 → active → 200。

## 3.3 对照实验：移除该请求头，确认 standby 立即恢复至默认的转发路径
...
预期再次收到 `200 OK` 与数据——同一 standby、同一客户端、同一 token，**仅因移除了该请求头，行为即从"307 重定向"切换回"透明转发"**。该对照实验同时验证：转发与重定向是 standby 在每次请求级别上做出的二选一决策，由请求头控制，与节点自身状态无关。
