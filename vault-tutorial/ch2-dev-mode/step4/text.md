...
| 场景 | 结论 |
| :--- | :--- |
| 本地开发机，只有你在用，数据可以随时丢失 | ✅ Dev 模式可以使用 |
| CI/CD 容器（隔离、短暂、测试结束即销毁） | ✅ Dev 模式可以使用 |
| 团队共享的开发服务器（多人访问） | ❌ 请使用生产模式（即使只是"开发用"） |
| 有任何数据需要在重启后保留 | ❌ 内存存储不满足要求 |
| 监听 `0.0.0.0`（对外暴露） | ❌ 无 TLS + 已知 Root Token = 立即被利用 |
| Staging / 生产 / 任何真实数据 | ❌🚫 永远不允许 |
