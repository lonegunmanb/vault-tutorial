...
echo "  /root/vault-s3.hcl         — storage \"s3\" { endpoint = localstack:4566 }"
echo "  /root/vault-dynamodb.hcl   — storage \"dynamodb\" { endpoint = localstack:4566 }"
echo ""
echo "便捷脚本："
echo "  ./start-vault.sh <file|inmem|pg|s3|dynamodb>   # 切换并重启 Vault"
echo "  ./start-postgres.sh                            # Step3 才需要"
echo "  ./start-localstack.sh                          # Step4 / Step5 才需要"
echo ""
echo "VAULT_ADDR 默认 = http://127.0.0.1:8200"
echo "AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY = test/test（已持久化，指向 LocalStack）"
echo ""
