...
还记得环境初始化时创建的 `app-team-a-read` Policy 吗？它引用的是 `secret/data/app-team-a/*`。目前 `secret/` 引擎还没动过，所以它依然正常。但来做一个实验——假如我们把 `secret/` 迁移到 `kv-prod/`：

```bash
echo "=== 迁移前：alice 用当前 policy 读取 ==="
ALICE_TOKEN=$(vault login -method=userpass -path=userpass \
  username=alice password=training -format=json | jq -r .auth.client_token)

VAULT_TOKEN=$ALICE_TOKEN vault kv get secret/app-team-a/db 2>&1 | head -5
...
再用 alice 的 token 试一次——这次走新路径：

```bash
echo "=== 迁移后：alice 尝试从新路径读取 ==="
VAULT_TOKEN=$ALICE_TOKEN vault kv get kv-prod/app-team-a/db 2>&1 | tail -5
export VAULT_TOKEN='root'
```

**403 Permission Denied**——因为 `app-team-a-read` Policy 里写的还是 `secret/data/app-team-a/*`，而引擎已经搬到 `kv-prod/` 了。
